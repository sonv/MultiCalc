...
>>>>>>> d475edf561c8eb9e0f7e529c2c82632697ba194d
\end{equation*}
...
From here, one can define higher partial derivatives.
...
<<<<<<< HEAD
\hypertarget{differentiability-and-tangent-planes}{%
\subsection{Differentiability and tangent planes}\label{differentiability-and-tangent-planes}}
...
``The'' \textbf{tangent plane} to the surface \(S\) at point \(P\) is defined the be the plane that contains
...
Let \(z = f(x,y)\) and \(\Delta z = f(a + \Delta x, b + \Delta y ) - f(a,b)\).
Then \(f\) is differentiable at \((a,b)\) if \(\Delta z\) can be
expressed in the form
\begin{equation*}
    \Delta z = \partial_x f(a,b) \Delta x + \partial_y f(a,b) \Delta y + \epsilon_1 \Delta x + \epsilon_2 \Delta y \,,
\end{equation*}
where \(\epsilon_1, \epsilon_2 \to 0\) as \((\Delta x, \Delta y) \to (0,0)\).
\end{definition}

It turns out that if \(f\) is differentiable at \((a,b)\), then there is only one
tangent plane to \(S\) at \((a,b, f(a,b)\).
=======
\hypertarget{tagent-planes-and-linear-approximations}{%
\subsection{Tagent planes and linear approximations}\label{tagent-planes-and-linear-approximations}}
>>>>>>> d475edf561c8eb9e0f7e529c2c82632697ba194d
