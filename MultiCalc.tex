...
\textbf{Reading: Stewart Chapter 12, Thomas Calculus Chapter 12, Active Calculus Chapter 9}

\hypertarget{functions-of-several-variables-1}{%
\subsection{Functions of several variables}\label{functions-of-several-variables-1}}

\(f: D \to C\) where \(D \subseteq \mathbb{R}^n\) and \(C \subseteq \mathbb{R}^m\).
\[f({x}) = f(x_1,\dots, x_n)\,.\]
...
\begin{definition}
A \textbf{vector function} (\textbf{vector-valued function}) is a function that has the codomain that belongs to \(\mathbb{R}^n\) where \(n\geq 2\).
\end{definition}
